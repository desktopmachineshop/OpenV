...
func (r *RepoConnectionRepository) Save(c *repoconns.RepoConnection) error {
	_, err := r.db.Exec(`
		INSERT INTO repo_connections (
			id, project_id, name, remote_url, default_branch,
...
func (r *RepoConnectionRepository) Update(c *repoconns.RepoConnection) error {
	_, err := r.db.Exec(`
		UPDATE repo_connections SET
			name = $2, remote_url = $3, default_branch = $4,
			credential_strategy = $5, updated_at = $6
		WHERE id = $1
	`,
		c.ID, c.Name, c.RemoteURL, c.DefaultBranch,
...
func (r *RepoConnectionRepository) FindByID(id string) (*repoconns.RepoConnection, error) {
	row := r.db.QueryRow(`
		SELECT id, project_id, name, remote_url, default_branch,
			credential_strategy, created_at, updated_at
...
// SetUserPath stores a user's local path for a connection; an empty path
// removes it.
...
// UserPaths returns a user's local paths for a project's connections, keyed
// by connection id.
