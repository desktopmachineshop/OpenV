...
request path with bounded concurrency; a push service answering 404 or 410
deletes the subscription, anything else stamps `failed_at` and keeps it. The
...
toggle, which says which of the three preconditions is missing (no keys on
the server, no support in the browser, permission denied) instead of failing
silently. The manifest gained two 1080×1920 phone screenshots of the review
