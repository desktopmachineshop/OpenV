...
Status: approved and implemented through Phase 3, 2026-09-07; Web Push
landed 2026-09-11 — see section 8 for what shipped and what is deferred. Evidence base: the frontend
...
### Web Push and manifest screenshots (2026-09-11)

The deferred first item shipped, closing REQ-109. Web push is opt-in
infrastructure twice over: nothing is sent unless the deployment has a VAPID
key pair (`make vapid-keys`, `OPENV_VAPID_PUBLIC_KEY` /
`OPENV_VAPID_PRIVATE_KEY` / `OPENV_VAPID_SUBJECT`), **and** a member has
granted permission on a device. Migration 0026 adds `push_subscriptions` —
one row per device, unique on the push service's endpoint, so a browser
rotating its keys refreshes the row rather than accumulating them — and
`users.push_notifications`, the per-user opt-in, default off. Four
session-only endpoints under `/api/v1/me` serve the config, the member's
devices, and subscribe / withdraw. The fan-out hangs off the same point as
the email side channel (`Notifier.deliver`, `BudgetMonitor`), carries the
same four high-signal types, and sends `{title, body, url, tag}` off the
request path on a fixed pool of 8 workers fed by a 1024-deep queue (a full
queue drops and counts rather than blocking the caller), each request bounded
by a 10s timeout; a push service answering 404 or 410 deletes the
subscription, anything else stamps `failed_at` and keeps it. `url` is a
same-origin PATH, because the service worker follows a tap with
`WindowClient.navigate`, which refuses a cross-origin URL (and falls back to
`openWindow` if it refuses anyway). A subscription endpoint is accepted only
at a known push-service host — `fcm.googleapis.com`, `*.push.apple.com`,
`*.notify.windows.com`, the Mozilla autopush hosts, plus whatever
`OPENV_PUSH_ENDPOINT_HOSTS` adds for a self-hosted service — never an address
literal, and no name is resolved: a subscribe endpoint is otherwise a
server-side request forgery primitive. The
service worker gained `push` and `notificationclick` handlers — the click
focuses an open window and navigates it rather than opening a second one —
and still caches nothing. Settings → Notifications carries the per-device
toggle, which reconciles the browser against the server — on only when this
device holds a subscription taken with the current VAPID key AND the server
lists its endpoint, with a re-register offered when it does not — and says
which precondition is missing (no keys on the server, no support in the
browser, no service worker, permission denied) instead of failing silently. The manifest gained two 1080×1920 phone screenshots of the review
queue and the board (generated by `phone-audit.js`, which learnt a
fixed-size screenshot mode for the purpose) plus the existing wide one, and
shortcuts to the review queue, the board and notifications: neither the
review queue nor the board has a project-free url, so those shortcuts land
on `/projects?go=…` and jump straight into the last project the member
opened, asking which project only when they have not opened one yet.
...
- Figure actions on touch and the tap-to-insert reference menu; server-side
  figure thumbnails; an iPad project and Lighthouse in CI.
